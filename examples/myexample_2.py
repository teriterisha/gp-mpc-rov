...
def ode_nominal(x, u):
    ...
    """
    dxdt = [
            u[0] - 0.5 * x[0] ** 2 - 1.1 * x[0],
            u[1] - 0.5 * x[1] ** 2 - 1.1 * x[1],
    ]
    return  dxdt

def ode_real(x, u):
    """ 2D ROV model，目前考虑了未知水动力，质量测量不准确，外界恒定干扰三个要素
    """
    dxdt = [
            (0.5 + u[0] - 0.5 * x[0] ** 2 - 1.1 * x[0] + 0.15 * x[1]) / 1.1,
            (0.4 + u[1] - 0.5 * x[1] ** 2 - 1.1 * x[1] + 0.15 * x[0]) / 1.1,
    ]
    return  dxdt
...
def ode_error(x, u):
    ...
    """
    dxdt = [
            (0.5 + u[0] - 0.5 * x[0] ** 2 - 1.1 * x[0] + 0.15 * x[1]) / 1.1 - (u[0] - 0.5 * x[0] ** 2 - 1.1 * x[0]),
            (0.4 + u[1] - 0.5 * x[1] ** 2 - 1.1 * x[1] + 0.15 * x[0]) / 1.1 - (u[1] - 0.5 * x[1] ** 2 - 1.1 * x[1]),
    ]
    return  dxdt
...
ulb = [-10, -10]    # input bound
uub = [10, 10]      # input bound
xlb = [-4., -4.]    # state bound
xub = [4., 4.]      # state bound
...
kine_fun_correct_ca = dyna_2_kine(my_gp_correct_fun_ca, Nx, Nu)
"""这部分代码是为了验证拓展后模型的正确性,验证结果还行，之后可以删去"""
ode_real_ca = get_ode_ca(ode_real, Nx, Nu)
kine_real_correct_ca = dyna_2_kine(ode_real_ca, Nx, Nu)
...
dx_test_extern_real = kine_real_correct_ca(x_test_extern, u_test)
dx_test_extern_predict = kine_fun_correct_ca(x_test_extern, u_test)
print(dx_test_extern_real, dx_test_extern_predict)
...
"""随后应该根据矫正过的模型进行预测控制，可以使用名义模型进行对比
   ...
   相较于example1加入避障功能,GPMPC效果略优于普通mpc（在当前设定条件下）
"""
Nx = 2 * Nx
N_pedict = 5 # 预测布长
obs_x = 0.5
...
state_l = [-2, -2, -4, -4, obs_r ** 2] 
state_u = [2, 2, 4, 4, np.inf] 
control_l = [-10, -10]   
control_u = [10, 10]    
...
for i in range(N_pedict):
    x_all_horizon[:, i + 1] = kine_fun_correct_rk4(x_all_horizon[:, i], u_all_horizon[:, i])
Q = np.eye(Nx)             # 状态惩罚矩阵
Q[0,0] = 10
Q[1,1] = 20
...
for i in range(N_pedict + 1):
    ...
    g.append((x_all_horizon[0, i] - obs_x) ** 2 + (x_all_horizon[1, i] - obs_y) ** 2)
...
# 仿真条件和相关变量
t0 = 0.0 # 仿真时间
x_init = np.array([-1.0, -1.0, 0.0, 0.0]).reshape(-1, 1) # 每一步的状态（未开始仿真前存储初始状态）
x_end = np.array([1.5, 1.5, 0.0, 0.0]).reshape(-1, 1)  # 末了状态
u_init = np.array([0.0, 0.0] * N_pedict).reshape(-1, Nu)    # nlp问题求解的初值，求解器可以在此基础上优化至最优值
...
while(np.linalg.norm(x_init - x_end) > 1e-4 and mpciter-sim_time / dt <0.0 ): 
    # 初始化优化参数
    ...
    # 获得最优控制结果u
    u_sol = ca.reshape(res['x'], Nu, N_pedict) # 将其恢复U的形状定义
...
# print(x_real_np,x_predict_np)
print(mpciter)
theta = np.linspace(0, 2 * np.pi, 200)
x_obs = np.cos(theta) * obs_r + obs_x
y_obs = np.sin(theta) * obs_r + obs_y

# print(index_t)
plt.figure()
plotsca_x = np.array([-1.0,1.5])
