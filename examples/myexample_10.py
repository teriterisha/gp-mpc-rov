...
ulb = [-100.0, -100.0, -100.0, -10.0]    # input bound
uub = [100.0, 100.0, 100.0, 10.0]      # input bound
xlb = [-2., -2., -2., -np.pi / 2]    # state bound
xub = [2., 2., 2., np.pi / 2]      # state bound
...
X, Y = my_generate_training_data(ode_error_4, N, Nu, Nx, uub, ulb, xub, xlb, noise_cov, noise=True)
# X_test, Y_test = my_generate_training_data(ode_error_4, N_test, Nu, Nx, uub, ulb, xub, xlb, noise_cov, noise=True)
...
ode_nominal_ca = get_ode_ca(ode_nominal_4, Nx, Nu)
...
kine_fun_correct_pre_ca = dyna_2_kine_nominal_4(my_gp_correct_fun_ca, Nx, Nu)
...
# 真实模型扩展+离散化
ode_real_ca = get_ode_ca_distrub(ode_real_4_distrub, x_distrub, y_distrub, 5, 4)
kine_real_ca = dyna_2_kine_real_4(ode_real_ca, 8, 4)
...
# 名义模型扩展+离散化
kine_nominal_ca = dyna_2_kine_nominal_4(ode_nominal_ca, Nx, Nu)
kine_nominal_rk4 = my_rk4_fun(kine_nominal_ca, dt, 2 * Nx, Nu)

ode_error_ca = get_ode_ca_distrub(ode_error_4_distrub, x_distrub, y_distrub, 5, 4)
...
state_l = [-np.inf, -np.inf, -np.inf, -np.inf, -4., -4., -4., -np.pi / 2] 
state_u = [np.inf, np.inf, np.inf, np.inf, 4., 4., 4., np.pi / 2] 
control_l = [-100.0, -100.0, -100., -10.0]   
control_u = [100.0, 100.0, 100., 10.0]    
...
state_ref = get_mpc_parameter_4(x_fun, y_fun, z_fun, yaw_fun, vx_fun, vy_fun, vz_fun, theta_fun, dt, t0, N_predict)
...
while(mpciter-sim_time / dt <0.0 ): 
    ...
    u_sub = u_this[0,:]
    # print(x_sub, u_sub)
    error_this = ode_error_ca(x_sub_t, u_sub, t0)
    # print(error_this)
    ...
    if(mpciter % 1 == 0):    
        ...
        my_gp_correct_fun_ca = merge_model(ode_nominal_ca, my_gp_error_fun_ca, 4, Nu)
        kine_fun_correct_pre_ca = dyna_2_kine_nominal_4(my_gp_correct_fun_ca, 4, Nu)
        kine_fun_correct_rk4 = my_rk4_fun(kine_fun_correct_pre_ca, dt, 2 * 4, Nu)
