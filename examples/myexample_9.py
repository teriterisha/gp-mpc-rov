...
# 模型准确但无干扰
ode_real_no_dis_ca = get_ode_ca(ode_real_4, 4, 4)
kine_real_no_dis_ca = dyna_2_kine_nominal_4(ode_real_no_dis_ca, Nx, Nu)
...
state_l = [-np.inf, -np.inf, -np.inf, -np.inf, -2., -2., -2., -np.pi / 2] 
state_u = [np.inf, np.inf, np.inf, np.inf, 2., 2., 2., np.pi / 2] 
...
# plt.scatter(x_real_np[:, 0], x_real_np[:, 1], s= 4, c='b')
