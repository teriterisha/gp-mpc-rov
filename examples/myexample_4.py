...
def x_fun(t):
    ...
    return math.sin(math.pi * t / 10) * 2
    # return t

def y_fun(t):
    return -math.cos(math.pi * t / 10) * 2 + 2.0
    # return 1
...
state_l = [-np.inf, -np.inf, -4 * np.pi, -4., -1.0, -np.pi / 2] 
state_u = [np.inf, np.inf, 4 * np.pi, 4., 1.0, np.pi / 2] 
control_l = [-50.0, -50.0, -1.0]   
control_u = [100.0, 100.0, 1.0]    
